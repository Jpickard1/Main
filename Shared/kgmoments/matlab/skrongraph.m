...
%
% key-value arguments:
%   nthreads : the number of computational threads to use [{numCores} | int]
%   nomex : do not attempt to use the mex version [{0} | 1]

try
    nthreads = feature('numCores');
catch M %#ok<NASGU>
    nthreads = 1;
    warning('skrongraph:numThreads',...
        'feature(''numCores'') did not work to find numThreads, running single threaded');
end    

optsu = struct(varargin{:});
opts = struct('symmetric',0,'noise',0,'nthreads',max(1,nthreads),'nomex',0);
...
if ~opts.nomex
    try
        % TODO optimize nthreads for small problems
        edgebuf = skrongraph_mex(T,r,opts.nthreads);
        A = sparse(edgebuf(:,1),edgebuf(:,2),1,N,N);
        return
    catch M %#ok<NASGU>
        warning('skrongraph:noMex',...
            'the mex version is not present, this will be VERY slow.');
    end
end
